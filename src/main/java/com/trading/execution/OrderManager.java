...
public class OrderManager {
    ...
    // ── Tracked position per composite key: token|strategyName ───────
    public record TrackedPosition(double entryPrice, int qty, String side, String symbol) {
        // backward-compat constructor without symbol
    ...
    public boolean buy(String symbol, String token, String exchange,
        ...
        try {
            ...
            String orderId = placeOrder(symbol, token, exchange, qty, "BUY", limitPrice);
            ...
            log.info("BUY filled | orderId:{} | {} x{} @ Rs.{}", orderId, symbol, qty, fmt(price));
            return true;
    ...
    public boolean sellShort(String symbol, String token, String exchange,
        ...
        try {
            ...
            String orderId = placeOrder(symbol, token, exchange, qty, "SELL", limitPrice);
            if (orderId == null) return false;
    ...
    public boolean closePosition(String symbol, String token, String exchange,
        ...
        try {
            ...
            String orderId = placeOrder(symbol, token, exchange, pos.qty(), closeSide, limitPrice);
            if (orderId == null) return false;

            boolean filled = waitForFill(orderId, limitWaitSeconds);
            if (!filled) {
                log.warn("Close order not filled in {}s for {} - will retry on next tick", limitWaitSeconds, symbol);
                return false;
            }
            ...
            log.info("Position closed | {} @ Rs.{} | P&L: Rs.{}", symbol, fmt(price), fmt(pnl));
            return true;
    ...
    public void placeStopLoss(String symbol, String token, String exchange,
        ...
        if (paperMode) return;  // Paper mode: SL handled in ExitHandler via price check
        ...
        try {
            ...
            if (resp != null && resp.orderId != null) {
                ...
                log.info("SL placed | {} | trigger:Rs.{} | orderId:{}", symbol, fmt(triggerPrice), resp.orderId);
            }
        } catch (Exception e) {
    ...
    /** Restore a position from DB on startup (no risk counters — already counted). */
    public void restorePosition(String positionKey, double entryPrice, int qty, String side, String symbol) {
        openPositions.put(positionKey, new TrackedPosition(entryPrice, qty, side, symbol));
        log.info("Position restored | key={} | {} {} x{} @ Rs.{}", positionKey, side, symbol, qty, fmt(entryPrice));
    }
    ...
    private String placeOrder(String symbol, String token, String exchange,
                               int qty, String side, double limitPrice) throws Exception {
    ...
    private boolean waitForFill(String orderId, int timeoutSeconds) {
        ...
        while (System.currentTimeMillis() < deadline) {
            try {
                ...
                // Most recent status is the last entry in history
    ...
    private static String posKey(String token, String strategy) { return token + "|" + strategy; }
}
